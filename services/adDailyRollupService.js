...
async function tryAcquireDistributedLock({ leaseMs = DEFAULT_LEASE_MS }) {
  ...
  try {
    ...
    logger.warn('Ad rollup lock error:', err.message);
    return false;
  }
}
...
function startDailyRollupScheduler({ intervalMs = 60 * 60 * 1000, lookbackDays = 35, leaseMs = DEFAULT_LEASE_MS } = {}) {
  ...
  const tick = async () => {
    ...
    try {
      ...
      logger.info(`Ad daily rollup refreshed (${affected} group rows)`);
    } catch (err) {
      logger.error('Ad daily rollup scheduler error:', err.message);
    } finally {
      running = false;
    }
  };
