...
const resolveWebsiteId = async (websiteParam) => {
  ...
  const website = await Website.findOne({ slug: String(websiteParam).trim().toLowerCase() }).select('_id').lean();
...
exports.list = asyncHandler(async (req, res) => {
  ...
  const [items, total] = await Promise.all([
    AdCreative.find(filter).populate('website', 'name slug').sort({ updatedAt: -1 }).skip(skip).limit(limit).lean(),
...
exports.getById = asyncHandler(async (req, res) => {
  const item = await AdCreative.findById(req.params.id).populate('website', 'name slug').lean();
