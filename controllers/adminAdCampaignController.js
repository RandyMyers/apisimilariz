...
/** Build { path, before, after } entries for moderation log (capped count/length). */
...
exports.list = asyncHandler(async (req, res) => {
  ...
  const [items, total] = await Promise.all([
    AdCampaign.find(filter).populate('website', 'name slug').sort({ createdAt: -1 }).skip(skip).limit(limit).lean(),
