...
const pickCreative = (creatives = []) => {
  ...
  // Legacy embedded creative (pre–AdCreative collection)
...
exports.getPlacements = asyncHandler(async (req, res) => {
  ...
  const locale = String(req.locale || req.query.locale || '').trim().toLowerCase();
  const device = String(req.query.device || '').trim().toLowerCase();
  let country = String(req.query.country || req.country || '').trim().toLowerCase();
...
const createEvent = (eventType) =>
  asyncHandler(async (req, res) => {
    ...
    const locale = String(req.locale || req.body.locale || '').trim().toLowerCase();
    ...
    if (!country) country = String(req.country || '').trim().toLowerCase();
    if (!country || !/^[a-z]{2}$/.test(country)) {
    ...
    const ip = (req.ip || req.connection?.remoteAddress || '').toString().trim().slice(0, 80);
